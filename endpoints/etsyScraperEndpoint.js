...
router.post('/bestsellerSearch', async function(req, res) {
    ...
    for (let searchTerm of searchTerms) {
        ...
        let mostCommonDescriptions = getMostCommonStrings(descriptions, numberSpinner);
        ...
        await new Promise((resolve, reject) => {
            fs.writeFile(filepath, output, 'utf8', function(err) {
                if (err) {
                    console.log(chalk.red(`An error occurred while writing to file at path: ${filepath}. Error details: ${err}`));
                    reject(err);
                } else {
                    console.log(chalk.green('.txt file has been created at: ' + filepath));
...
router.post('/normalSearch', async function(req, res) {
    ...
    for (let searchTerm of searchTerms) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            // Check if the page has been redirected
            const currentPageUrl = page.url();
            if (i !== 0 && currentPageUrl !== `https://www.etsy.com/search?q=${encodedSearchTerm}&ref=pagination&page=2&ship_to=GB`) {
                console.log(chalk.yellow(`For "${searchTerm}", page 2 does not exist`));
                break; // Break the 2x loop
            }
