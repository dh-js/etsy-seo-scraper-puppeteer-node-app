...
router.post('/', async function(req, res) {
    var searchTerms = req.body.searchTerms.split('\n')
        .map(term => term.trim())
        .filter(Boolean);
    ...
    if (lines[0] === '**ENTER WORDS TO EXCLUDE BELOW, THIS IS NOT CASE SENSITIVE**') {
        lines = lines.slice(1);
    }
    // Split the remaining lines into an array of words
    let excludeWords = lines.join('\n').split(/\s+/).map(word => word.trim());

    console.log("Starting Etsy Scraping.")

    // Log each search term
    searchTerms.forEach((term, index) => {
        console.log(`Search term ${index + 1}: ${term}`);
    });
    ...
    for (let searchTerm of searchTerms) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            await page.waitForTimeout(Math.floor(Math.random() * 7000) + 7000);
            ...
            if (i !== 0 && currentPageUrl !== `https://www.etsy.com/search?q=${encodedSearchTerm}&explicit=1&is_best_seller=true&ship_to=GB&page=2&ref=pagination`) {
                console.log(`For "${searchTerm}", page 2 does not exist`);
            ...
            if (tooManyRequestsElement) {
                const textContent = await page.evaluate(element => element.textContent, tooManyRequestsElement);
                if (textContent.trim() === 'We have received too many requests from you recently! Please try again later.') {
            ...
            console.log(`For "${searchTerm}", the number of listings on page ${i + 1} is: ${pageListings.length}`);
            ...
            for (let listing of pageListings) {
                await page.goto(listing, {waitUntil: 'networkidle0'});
                await page.waitForTimeout(Math.floor(Math.random() * 6000) + 6000);
                ...
                let title = await page.evaluate(() => {
                    const titleElement = document.querySelector('h1[data-buy-box-listing-title="true"]');
                ...
                let tags = await page.evaluate(() => {
                    const tagsElement = document.querySelector('div[data-appears-component-name="Listzilla_ApiSpecs_Tags_WithImages"]');
                    if (tagsElement) {
                        const tagsData = JSON.parse(tagsElement.getAttribute('data-appears-event-data'));
                        return tagsData.queries;
                    }
                    return null;
                });
                if (tags) {
                    tags = tags.map(tag => cleanseText(tag));
                ...
                let altTexts = await page.evaluate((title) => {
                    const imageElements = document.querySelectorAll('ul[data-carousel-pagination-list] img');
        ...
        let titles = listingDataForSearchTerm.flatMap(listing => typeof listing.title === 'string' ? listing.title.split(' ') : [])
            .filter(word => word && isNaN(word) && word.trim() !== '');

        let tags = listingDataForSearchTerm.flatMap(listing => Array.isArray(listing.tags) ? listing.tags : [])
            .filter(word => word && isNaN(word) && word.trim() !== '');

        let descriptions = listingDataForSearchTerm.flatMap(listing => typeof listing.description === 'string' ? listing.description.split(' ') : [])
            .filter(word => word && isNaN(word) && word.trim() !== '');

        let altTexts = listingDataForSearchTerm.flatMap(listing => Array.isArray(listing.alttext) ? listing.alttext.flatMap(alttext => typeof alttext === 'string' ? alttext.split(' ') : []) : [])
            .filter(word => word && isNaN(word) && word.trim() !== '');
        ...
        function getMostCommonStrings(arr, limit) {
            let counts = arr.reduce((acc, str) => {
                ...
                if (!excludeWords.map(word => word.toLowerCase()).includes(str.toLowerCase())) {
                    acc[str] = (acc[str] || 0) + 1;
                }
                return acc;
            }, {});
        ...
        let mostCommonTitles = getMostCommonStrings(titles, 20);
        let mostCommonTags = getMostCommonStrings(tags, 20);
        let mostCommonDescriptions = getMostCommonStrings(descriptions, 20);
        let mostCommonAltTexts = getMostCommonStrings(altTexts, 20);
        let mostCommonCategories = getMostCommonStrings(categories, 20);

        console.log(`Completed data analysis for "${searchTerm}"`)
        ...
        mostCommonTitles.forEach((title, index) => output += `    ${index + 1}. ${title}\n`);
        output += '\nMost Common Tags:\n';
        mostCommonTags.forEach((tag, index) => output += `    ${index + 1}. ${tag}\n`);
        output += '\nMost Common Description Keywords:\n';
        mostCommonDescriptions.forEach((description, index) => output += `    ${index + 1}. ${description}\n`);
        output += '\nMost Common Alt Text Keywords:\n';
        mostCommonAltTexts.forEach((altText, index) => output += `    ${index + 1}. ${altText}\n`);
        output += '\nMost Common Categories:\n';
        mostCommonCategories.forEach((category, index) => output += `    ${index + 1}. ${category}\n`);
        output += '\n\n';
        ...
        listingDataForSearchTerm.forEach((listing, index) => {
            ...
            output += `    Tags:\n`;
            listing.tags ? listing.tags.forEach(tag => output += `        - ${tag}\n`) : output += `        - None\n`;
            output += `\n    Description:\n        ${listing.description}\n\n`;
            output += `    Alt Text:\n`;
            listing.alttext ? listing.alttext.forEach(alt => output += `        - ${alt}\n`) : output += `        - None\n`;
            output += `\n    Category:\n        ${listing.category}\n\n\n\n`;
        });
        ...
        await new Promise((resolve, reject) => {
            fs.writeFile(filepath, output, 'utf8', function(err) {
                if (err) {
                    console.log(`An error occurred while writing to file at path: ${filepath}. Error details: ${err}`);
                    reject(err);
                } else {
                    console.log('.txt file has been created at: ' + filepath);
                    successfulSearchTerms.push(searchTerm);
                    // Remove the successful search term from the unsuccessfulSearchTerms array
                    unsuccessfulSearchTerms = unsuccessfulSearchTerms.filter(term => term !== searchTerm);
                    resolve();
                }
            });
        });
